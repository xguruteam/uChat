...
//  uChat-project
//
//  Created by Luccas Beck on 4/4/19.
...
class UserProfileDataDatabaseUpdater: NSObject {
  ...
  func deleteCurrentPhoto(completion: @escaping DeleteCurrentPhotoCompletionHandler) {
    ...
    userReference.observeSingleEvent(of: .value, with: { (snapshot) in
      ...
      guard let photoURL = userData["photoURL"] as? String , let thumbnailPhotoURL = userData["thumbnailPhotoURL"] as? String, photoURL != "", thumbnailPhotoURL != "" else {
        completion(true)
        return
      }
